...
function selectBot() {
    if (poe_settings.bot) {
        $("#poe_bots")
            .find(`option[value="${poe_settings.bot}"]`)
            .attr("selected", true);
    }
}
...
function appendSuggestedReply(reply) {
    ...
    const newElement = $(
        `<div class="suggested_reply"><div class="suggested_reply_text">${reply}</div></div>`
    );
    newElement.hide();
        ...
        if (power_user.prefer_character_jailbreak && jailbreakPrompt) {
            prompt +=
                ...
                substituteParams(
                    ...
                    name1,
                    name2,
                    poe_settings.character_nudge_message
                );
        } else {
            prompt +=
...
async function autoJailbreak() {
    for (
        ...
        const reply = await sendMessage(
            substituteParams(poe_settings.jailbreak_message),
            false,
            false
        );
...
async function getPoeTokenMessagesLeft() {
    const response = await fetch("/poe_messages_left", {
        headers: getRequestHeaders(),
        method: "POST",
    });
    ...
    if (data.hasMessagesLeft) return true;

    return false;
}

async function generatePoe(type, finalPrompt, signal) {
    // if the response is false, then the token will be
    // changed with the next message, hence forcing us to be prepared to JB again
    auto_jailbroken = auto_jailbroken && (await getPoeTokenMessagesLeft());
...
async function onBotAddClick() {
    let botName = $("#poe_add_bot_name").val().trim();
    ...
    $("#poe_add_bot").css("display", "none");
    $("#poe_add_bot_loading").css("display", "inline-block");

    const body = JSON.stringify({ botToAdd: botName });

    const response = await fetch("/add_poe_bot", {
        headers: getRequestHeaders(),
        body: body,
        method: "POST",
    });
    ...
    if (!data.ok) {
        $("#poe_add_bot").css("display", "block");
        $("#poe_add_bot_loading").css("display", "none");
...
$("document").ready(function () {
    ...
    $("#poe_token_length").on("input", onPoeTokenLengthInput);
